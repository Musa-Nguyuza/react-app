...
const ProcessQa = () => {
  ...
  const {id} = useParams();
  const { tableData } = useFetchTableData(id, 'Process Table');
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setSelectedFilters({ ...parsed, grouping: 'Process' });
    } else {
      setSelectedFilters({ filter: '', grouping: 'Process' });
    }
  }, []);
  ...
  const handleChange = (index, field, value) => {
    const updated = [...processTableData];
    updated[index][field] = value;
    setProcessTableData(updated);
  };
  ...
  const handleSubmit = async () => {
    ...
    // const isComplete = processTableData.every((row) => {
  ...
  const paginatedRows = processTableData.slice(
    page * rowsPerPage,
  ...
  return (
    <>
      ...
      <TablePagination
        component="div"
        count={processTableData.length} /* ✅ Use context data count */
        page={page}
