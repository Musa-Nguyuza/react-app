...
const QaCustomerExp = () => {
  ...
  const {id} = useParams();
  const { tableData } = useFetchTableData(id, 'Customer Experience Table');

  const dataSource = id ? tableData : data;

  // ✅ Using context data instead of local state
  const { customerExperienceTableData, setCustomerExperienceTableData, setCustomerSummary,customerSummary } = useContext(CallLogContext);
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setSelectedFilters({ ...parsed, grouping: 'Customer Experience' });
    } else {
      setSelectedFilters({ filter: '', grouping: 'Customer Experience' });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if(customerExperienceTableData.length === 0){
    ...
    setCustomerExperienceTableData(id? dataSource : mappedData);
  };
  }, [customerExperienceTableData, dataSource, id, selectedFilters, setCustomerExperienceTableData]);

  useEffect(()=>{
    
    // ✅ Count compliant values
    const summary = {Yes: 0, No: 0 , 'N/A': 0};
    customerExperienceTableData.forEach((row)=>{
  ...
  const handleChange = (index, field, value) => {
    const updated = [...customerExperienceTableData];
    updated[index][field] = value;
    setCustomerExperienceTableData(updated);
  };
  ...
  const handleSubmit = async () => {
    ...
    // const isComplete = customerExperienceTableData.every((row) => {
    ...
    setTableError(false);
    navigate(id? `/Remediation/${id}` :'/Remediation');

  };

  // ✅ Use context data for pagination
  const paginatedRows = customerExperienceTableData.slice(
    page * rowsPerPage,
  ...
  return (
    <>
      ...
      <TableContainer component={Paper} sx={{ maxHeight: 700, mt: 2 }}>
        <Table stickyHeader>
          ...
          <TableHead sx={{
            ...
            <TableRow>
              <TableCell>
                <strong>Total Phrases: {customerExperienceTableData.length}</strong>
              </TableCell>
              <TableCell>
                <strong>Total compliant Phrases: {customerSummary['Yes']}</strong>
              </TableCell>
              <TableCell>
                <strong>Total Non-Compliant: {customerSummary['No']}</strong>
              </TableCell>
              <TableCell>
                <strong>Total Completed Phrases: {customerExperienceTableData.length}</strong>
              </TableCell>
            </TableRow>
          </TableHead>
        </Table>
      ...
      <TablePagination
        component="div"
        count={customerExperienceTableData.length} /* ✅ Use context data count */
        page={page}
